...
#this is used only for the Real World Vidoes
...
class PINN_Pendulum(nn.Module):
    def __init__(self, hidden_dim=20):
        super(PINN_Pendulum, self).__init__()
...
def physics_loss_pendulum(model, t_interior, g=9.8, length=1.0):
...
def train_pinn_pendulum(T=2.0, g=9.8, length=1.0, n_phys_points=200, t_data=None, y_data=None,
    ...
    model = PINN_Pendulum(hidden_dim=20)
    ...
    for epoch in range(n_epochs):
        optimizer.zero_grad()
        p_loss = physics_loss_pendulum(model, t_interior, g=g, length=length)
...
def load_trajectories_pendulum(base_directory, file_name):
    trajectories = []
    for traj_num in range(200):
        ...
        # t_data, x_data = apply_filter(t_data, x_data, kernel_size=25)
        trajectories.append((t_data, x_data))
    return trajectories

def train_and_evaluate_pendulum(trajectories, output_dir):
    results = []
    mse_list, nmse_list = [], []
    ...
    for i, (t_data, x_data) in enumerate(trajectories):
        ...
        model = train_pinn_pendulum(T=2.0, g=9.8, length=1.0, n_phys_points=200,
                                    t_data=t_data, y_data=x_data, n_epochs=200000, lr=1e-3)
...
class PINN_FreeFall(nn.Module):
    def __init__(self, hidden_dim=20):
        super(PINN_FreeFall, self).__init__()
        self.net = nn.Sequential(
            nn.Linear(1, hidden_dim),
            nn.Tanh(),
            nn.Linear(hidden_dim, hidden_dim),
...
def physics_loss_freefall(model, t_interior, g=9.8):
    t_interior.requires_grad_(True)
...
def train_pinn_freefall(T=2.0, g=9.8, n_phys_points=50, t_data=None, y_data=None,
                        n_epochs=5000, lr=1e-3, early_stop_patience=500, early_stop_min_delta=1e-6):
    t_interior = torch.linspace(0, T, n_phys_points).reshape(-1, 1)
    model = PINN_FreeFall(hidden_dim=20)
    optimizer = optim.Adam(model.parameters(), lr=lr)
    ...
    for epoch in range(n_epochs):
        optimizer.zero_grad()
        p_loss = physics_loss_freefall(model, t_interior, g=g)
        d_loss = data_loss(model, t_data, y_data)
        loss_value = p_loss + d_loss
        loss_value.backward()
        optimizer.step()
...
def load_trajectories_freefall(base_directory, file_name):
    trajectories = []
    for traj_num in range(200):
        video_dir = os.path.join(base_directory, f"video_{traj_num}")
        file_path = os.path.join(video_dir, file_name)
...
def train_and_evaluate_freefall(trajectories, output_dir):
    results = []
    mse_list, nmse_list = [], []
    os.makedirs(output_dir, exist_ok=True)
    ...
    for i, (t_data, x_data) in enumerate(trajectories):
        ...
        model = train_pinn_freefall(T=2.0, g=9.8, n_phys_points=50,
                                    t_data=t_data, y_data=x_data, n_epochs=200000, lr=1e-3)
...
class PINN_Projectile(nn.Module):
    def __init__(self, hidden_dim=20):
        super(PINN_Projectile, self).__init__()
        self.net = nn.Sequential(
            nn.Linear(1, hidden_dim),
            nn.Tanh(),
            nn.Linear(hidden_dim, hidden_dim),
...
def physics_loss_projectile(model, t_interior, g=9.8):
    t_interior.requires_grad_(True)
...
def data_loss_projectile(model, t_data, x_data, y_data):
    pred = model(t_data)
    x_pred = pred[:, 0:1]
    y_pred = pred[:, 1:2]
    return torch.mean((x_pred - x_data)**2 + (y_pred - y_data)**2)

def train_pinn_projectile(T=2.0, g=9.8, n_phys_points=50, t_data=None, x_data=None, y_data=None,
                          n_epochs=5000, lr=1e-3, early_stop_patience=500, early_stop_min_delta=1e-6):
    t_interior = torch.linspace(0, T, n_phys_points).reshape(-1, 1)
    model = PINN_Projectile(hidden_dim=20)
    optimizer = optim.Adam(model.parameters(), lr=lr)
    ...
    for epoch in range(n_epochs):
        optimizer.zero_grad()
        p_loss = physics_loss_projectile(model, t_interior, g=g)
        d_loss = data_loss_projectile(model, t_data, x_data, y_data)
        loss_value = p_loss + d_loss
        loss_value.backward()
        optimizer.step()
...
def load_trajectories_projectile(base_directory, file_name, kernel_size=11, apply_smoothing=True):
    trajectories = []
    for traj_num in range(200):
        video_dir = os.path.join(base_directory, f"video_{traj_num}")
        file_path = os.path.join(video_dir, file_name)
...
def train_and_evaluate_projectile(trajectories, output_dir):
    results = []
    mse_list, nmse_list = [], []
    os.makedirs(output_dir, exist_ok=True)
    ...
    for i, (t_data, x_data, y_data) in enumerate(trajectories):
        ...
        model = train_pinn_projectile(T=2.0, g=9.8, n_phys_points=50,
                                      t_data=t_data, x_data=x_data, y_data=y_data,
...
def run_pin_framework(base_directory, file_name="3D_centers.pkl", phenomenon="pendulum"):
    ...
    if phenomenon == "pendulum":
        ...
        trajectories = load_trajectories_pendulum(base_directory, file_name)
        trajectories = [(t, x - torch.mean(x)) for t, x in trajectories]
        train_and_evaluate_pendulum(trajectories, output_dir)
    elif phenomenon == "freefall":
        print("[Framework] Running Free Fall PINN.")
        trajectories = load_trajectories_freefall(base_directory, file_name)
        train_and_evaluate_freefall(trajectories, output_dir)
    elif phenomenon == "projectile":
        print("[Framework] Running Projectile PINN.")
        trajectories = load_trajectories_projectile(base_directory, file_name, kernel_size=11, apply_smoothing=True)
        train_and_evaluate_projectile(trajectories, output_dir)
    else:
